  ...
  const regelModal = regeln && (
    <div className="overlay" onClick={() => setRegeln(false)}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        <h2>Traumelf</h2>
        <p className="ruleP">Jeder Spin zieht einen <b>echten Verein in einer echten Saison</b>. Nimm daraus <b>einen</b> Spieler auf eine freie Position — elfmal, bis die Elf steht. Danach spielt sie eine volle Saison in einer <b>echten Liga</b> — {LIGEN.map((l) => `${LIGA_NAME[l]} ${saisonName(NEUESTE_SAISON[l])}`).join(", ")} — und nimmt dort den Platz eines der Vereine ein.</p>
